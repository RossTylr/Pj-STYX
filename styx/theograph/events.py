...
def expand_history(patient: Patient) -> tuple[CareEvent, ...]:
    """Materialise ``patient.theograph`` counts into a deterministic, recency-biased event timeline."""
    rng = np.random.default_rng((SEED, patient.pid))  # explicit vector seed → DET-1
    events: list[CareEvent] = []
    for channel in CHANNELS:  # fixed order — never dict order
        n = int(patient.theograph[channel])
        if n <= 0:
            continue
        t_days = -LIFELONG_DAYS * (rng.random(n) ** RECENCY_POWER)
        for td in t_days:
            in_ep = bool(td >= -EPISODE_DAYS)
            t_min = (td + EPISODE_DAYS) / EPISODE_DAYS * _EPISODE_SPAN_MIN if in_ep else float("nan")
            events.append(CareEvent(channel, float(td), in_ep, float(t_min)))
