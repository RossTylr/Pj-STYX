...
gap read from the real fire-points (305 min on the silent case — the headline pin); the comparator
lane is ``styx.readouts.news2_partial`` passed through unchanged (fair NEWS2 — never re-scored or
weakened inside the figure); and no crossing marker or lead bracket appears before the replay
clock has reached it (rule 6).
"""
...
def _figure(patient, ctx, now_idx: int | None = None):
    return comparison_figure(
        ...
        news2_crossing_min=news2_crossing(patient), now_idx=now_idx)


def test_lead_annotation_single_sourced_from_fire_points() -> None:
    patient, ctx = _setup()
    lead = news2_crossing(patient) - ctx.fire.aegis_min
...
def test_news2_lane_is_the_readout_unchanged() -> None:
    ...
    assert np.array_equal(np.asarray(lane.y), news2_partial(patient))
